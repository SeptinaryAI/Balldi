...
def main():
    ...
    send_cmd(dll, CMD_SET_PROG_RESIS, 7)

    send_cmd(dll, CMD_GET_BAT_VOLTAGE, 0)
    send_cmd(dll, CMD_GET_CHRG_VOLTAGE, 0)
    send_cmd(dll, CMD_GET_PROG_RESIS, 0)
    send_cmd(dll, CMD_GET_IS_CHAR, 0)
    send_cmd(dll, CMD_GET_IS_STDBY, 0)

    for i in range(5):
        send_cmd(dll, CMD_SET_LEFT_PWM, 30)
        send_cmd(dll, CMD_SET_RIGHT_PWM, 30)
        time.sleep(0.4)
        send_cmd(dll, CMD_SET_LEFT_PWM, -30)
        send_cmd(dll, CMD_SET_RIGHT_PWM, -30)
        time.sleep(0.4)
        send_cmd(dll, CMD_SET_LEFT_PWM, 0)
        send_cmd(dll, CMD_SET_RIGHT_PWM, 0)
        time.sleep(0.5)
        send_cmd(dll, CMD_SET_FLY_PWM, 40)
        time.sleep(0.4)
        send_cmd(dll, CMD_SET_FLY_PWM, -40)
        time.sleep(0.4)
